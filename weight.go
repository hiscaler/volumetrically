...
func (w Weight) Kilogram(precision int32) float64 {
	return w.value.Div(decimal.NewFromInt(1000)).Round(precision).InexactFloat64()
}

// Pound 磅，precision表示小数点位数
func (w Weight) Pound(precision int32) float64 {
	return w.value.Div(decimal.NewFromFloat(453.59237)).Round(precision).InexactFloat64()
}
